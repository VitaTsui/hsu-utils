...
describe('getDateRange', () => {
  ...
  test('hasTime 为 true 时，past 类型 day 单位包含时间部分', () => {
    const baseDate = dayjs('2024-03-20')
    const result = getDateRange({
      type: 'past',
      amount: 7,
  ...
  test('hasTime 为 true 时，future 类型 week 单位包含时间部分', () => {
    const baseDate = dayjs('2024-03-20')
    const result = getDateRange({
      type: 'future',
      amount: 2,
  ...
  test('hasTime 为 true 时，thisWeek 类型包含时间部分', () => {
    const baseDate = dayjs('2024-03-20')
    const result = getDateRange({
      type: 'thisWeek',
      baseDate: baseDate.toDate(),
    ...
    const [min, max] = result
    expect(min).toBe(baseDate.startOf('week').format('YYYY-MM-DD HH:mm:ss'))
    expect(max).toBe(baseDate.endOf('week').format('YYYY-MM-DD HH:mm:ss'))
  })

  test('hasTime 为 true 时，future 类型 day 单位包含时间部分（覆盖 baseFormat === YYYY-MM-DD 条件）', () => {
    const baseDate = dayjs('2024-03-20')
    const result = getDateRange({
      type: 'future',
      amount: 5,
    ...
    const expectedMin = baseDate.startOf('day')
    const expectedMax = baseDate.add(5, 'day').endOf('day')
  ...
  test('hasTime 为 true 时，past 类型 week 单位包含时间部分（覆盖 baseFormat.includes(DD) 条件）', () => {
    const baseDate = dayjs('2024-03-20')
    const result = getDateRange({
      type: 'past',
      amount: 3,
    ...
    const expectedMin = baseDate.subtract(3, 'week').startOf('week')
    const expectedMax = baseDate.endOf('week')
  ...
  test('hasTime 默认为 false', () => {
    ...
    const [min, max] = result
    expect(min).toBe(baseDate.startOf('week').format('YYYY-MM-DD'))
    expect(max).toBe(baseDate.endOf('week').format('YYYY-MM-DD'))
  })
